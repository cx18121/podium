...
**Plans**: 1 plan
Plans:
- [ ] 11-01-PLAN.md — computeFillerBreakdown() TDD pure function + FillerBreakdown component + Review.tsx wiring (ANAL-04, ANAL-05)
...
**Goal:** A one-time 30-second guided calibration recording measures the user's natural gaze offset, hand-to-nose distance, and shoulder sway, computing per-user thresholds stored in Dexie v4 so that scoring is accurate for each person's body and camera setup instead of relying on hardcoded heuristic constants.
**Requirements**: CAL-01, CAL-02, CAL-03, CAL-04, CAL-05, CAL-06
**Depends on:** Phase 13
**Success Criteria** (what must be TRUE):
  1. User can run a guided 30-second calibration from SetupScreen that measures their natural gaze, posture, and hand distance baselines
  2. Calibration profile (gazeThreshold, faceTouchThreshold, swayThreshold) is persisted in Dexie v4 calibrationProfiles table
  3. Subsequent recording sessions use calibrated thresholds instead of hardcoded defaults (0.15, 0.12, 0.04)
...
- [ ] 14-02-PLAN.md — CalibrationScreen UI with guided 2-step flow, worker calibration messages, App.tsx state machine + SetupScreen wiring
- [ ] 14-03-PLAN.md — Human verification: end-to-end calibration flow in browser
